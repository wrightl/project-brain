...
|------|--------|-----------|------|
| Users | `testuser1@projectbrain.test` … `testuser5@projectbrain.test` | No | `user` |
| Coaches | `sarah.mitchell@projectbrain.test`, `james.okonkwo@projectbrain.test`, … (10 fixed names) | Yes | `coach` |

Test coaches are created with real UK postcodes and latitude/longitude (one per city) so distance-based coach search can be exercised in dev and staging.

- **Local:** Set `TestUsers:Enabled` to `true` in `appsettings.Development.json` and provide `TestUsers:Password` via Aspire parameter `test-users-password` (same value as admin password is fine) or user secrets.
